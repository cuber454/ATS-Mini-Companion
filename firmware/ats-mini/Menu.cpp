...
static void doBleMode(int16_t enc)
{
  ...
  // If activating BLE, disable WiFi to avoid conflicts
  if(newBleModeIdx != BLE_OFF && bleModeIdx == BLE_OFF) {
    // Turning BLE ON - disable WiFi first
    netInit(NET_OFF);
    wifiModeIdx = NET_OFF;
  }
...
static void clickWiFiMode(uint8_t mode, bool shortPress)
{
  ...
  // If activating WiFi, disable BLE to avoid conflicts
  if(mode != NET_OFF && wifiModeIdx == NET_OFF) {
    // Turning WiFi ON - disable BLE first
    bleInit(BLE_OFF);
    bleModeIdx = BLE_OFF;
  }
